...
def build_system_prompt(slot: str, paragraph_count: int) -> str:
    ...
    return dedent(
        ...
        В данных есть раздел `story_order`. Это уже отсортированный список новостей для выпуска.
        Используй именно новости из `story_order` и строго в том порядке, в котором они даны.
        Не добавляй новости вне `story_order` и не меняй порядок.
        ...
        Каждый абзац должен описывать ровно одну новость и начинаться в формате:
        <эмодзи> <короткий заголовок>:
        После двоеточия дай 1-2 плотных предложения на русском: что произошло и почему это важно.
        Не делай сводных блоков вида "Dev tools" или "Короткой строкой". Каждая новость должна быть отдельным абзацем.
        ...
        Не добавляй вторую строку-подзаголовок, слоганы, сводные списки или дополнительные CAPS-фразы внутри абзаца.
        После двоеточия сразу идет обычный связный текст.
        Не вставляй URL в текст.
        Не используй markdown, списки и разделитель " | ".
